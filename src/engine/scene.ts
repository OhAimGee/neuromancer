import { Application, Container } from 'pixi.js';
import decors from '@data/decors.json';
import { Atmosphere, type Ambiance } from './atmosphere';
import { construireMatrice } from './matrice';
import { Plans, type PlanDecor } from './plans';
import { chargerTilemap } from './tilemap';

export const LARGEUR = 320;
export const HAUTEUR = 180;

interface ConfigDecor extends Ambiance {
  plans?: PlanDecor[];
  pluie?: boolean;
}

const DECORS = decors as unknown as Record<string, ConfigDecor | undefined>;

interface Assemblage {
  couche: Container;
  plans: Plans | null;
  atmosphere: Atmosphere | null;
}

/**
 * Le canvas de jeu, pilote en imperatif.
 *
 * React ne touche jamais a ce qui suit : il monte l'hote une fois, puis se
 * contente d'appeler afficherDecor(). Le rendu se fait reellement en 320x180
 * avec resolution: 1 — c'est le navigateur qui agrandit le canvas en plus
 * proche voisin. Un filtre CRT/glitch posé sur le stage s'executerait donc sur
 * ces 320x180 pixels, et non sur des pixels deja agrandis.
 *
 * Un decor n'est plus une tilemap mais un ASSEMBLAGE : un fond (des plans de
 * parallaxe, ou une tilemap), et par-dessus une atmosphere commune aux deux.
 * C'est ce qui separe un interieur carrele d'une rue qui a de la profondeur
 * sans obliger a choisir une seule technique pour tout le jeu.
 */
export class SceneJeu {
  private app: Application | null = null;
  private detruit = false;
  private decorActuel: string | null = null;
  private jeton = 0;
  private cache = new Map<string, Assemblage>();
  /** Decor reclame avant que le canvas ne soit pret. */
  private decorEnAttente: string | null = null;
  private montee = false;
  private courant: Assemblage | null = null;

  async monter(hote: HTMLElement): Promise<void> {
    const app = new Application();
    await app.init({
      width: LARGEUR,
      height: HAUTEUR,
      resolution: 1,
      antialias: false,
      roundPixels: true,
      backgroundAlpha: 0,
      preference: 'webgl',
    });
    // Le montage est asynchrone, et React 19 monte deux fois les effets en
    // mode strict : detruire() a pu passer avant que init() ne rende la main.
    if (!this.detruit && this.app === null && hote.isConnected) {
      this.app = app;
      this.montee = true;
      hote.appendChild(app.canvas);

      // La derive des plans et le battement des halos sont un mouvement
      // continu en plein ecran : ils tombent exactement sous la regle
      // d'accessibilite du projet, au meme titre que le glitch.
      if (!SceneJeu.mouvementReduit()) {
        app.ticker.add((horloge) => {
          const dt = horloge.deltaMS / 1000;
          this.courant?.plans?.animer(dt);
          this.courant?.atmosphere?.animer(dt);
        });
      }

      // React demande souvent le decor avant que init() n'ait rendu la main.
      // Sans ce rattrapage la demande etait perdue, et comme decorActuel avait
      // deja ete note, aucune demande identique ne repassait ensuite : le
      // decor restait noir pour toute la scene.
      if (this.decorEnAttente !== null) {
        const attendu = this.decorEnAttente;
        this.decorEnAttente = null;
        this.decorActuel = null;
        void this.afficherDecor(attendu);
      }
    } else {
      app.destroy(true);
    }
  }

  private static mouvementReduit(): boolean {
    return (
      typeof window !== 'undefined' &&
      typeof window.matchMedia === 'function' &&
      window.matchMedia('(prefers-reduced-motion: reduce)').matches
    );
  }

  /** Racine du canvas, pour les couches que la scene ne gere pas elle-meme. */
  racine(): Container | null {
    return this.app?.stage ?? null;
  }

  /** Attache une couche construite ailleurs (le graphe du cyberespace). */
  poser(couche: Container): boolean {
    if (!this.app || this.detruit) return false;
    this.app.stage.addChild(couche);
    return true;
  }

  private async assembler(id: string): Promise<Assemblage> {
    const conf = DECORS[id] ?? {};
    const couche = new Container({ label: id });

    let plans: Plans | null = null;
    if (conf.plans && conf.plans.length > 0) {
      plans = await Plans.construire(conf.plans, LARGEUR, HAUTEUR);
      couche.addChild(plans.couche);
    } else if (id === 'matrice') {
      couche.addChild(construireMatrice());
    } else {
      couche.addChild(await chargerTilemap(id));
    }

    const atmosphere = Atmosphere.construire(conf, LARGEUR, HAUTEUR);
    couche.addChild(atmosphere.couche);

    return { couche, plans, atmosphere };
  }

  async afficherDecor(id: string | null): Promise<void> {
    if (!this.montee) {
      this.decorEnAttente = id;
      return;
    }
    if (id === this.decorActuel) return;
    this.decorActuel = id;

    const jeton = ++this.jeton;
    // 'aucun' est un decor a part entiere : le noir. Sans lui, une scene sans
    // tag bg heriterait du decor precedent, faute de pouvoir l'effacer.
    const assemblage =
      id === null || id === 'aucun'
        ? null
        : (this.cache.get(id) ?? (await this.assembler(id)));
    // Un decor demande plus recemment a pris la main pendant le chargement.
    if (jeton !== this.jeton || !this.app || this.detruit) return;
    if (id && assemblage) this.cache.set(id, assemblage);

    this.app.stage.removeChildren();
    this.courant = assemblage;
    if (assemblage) this.app.stage.addChild(assemblage.couche);
  }

  detruire(): void {
    this.detruit = true;
    this.app?.destroy(true, { children: true });
    this.app = null;
    this.cache.clear();
    this.courant = null;
    this.decorActuel = null;
  }
}
